...
|---------|--------------|---------|
| `pwd` | **P**rint **W**orking **D**irectory - shows current location | `pwd` → `/Users/alice/ATOC4815` |
| `ls` | **L**i**s**t files and folders in current directory | `ls` → `homework1.py  lab1.ipynb  data/` |
...
## 🚨 CRITICAL: Handling Spaces in Folder Names {.smaller}
...
- If keys aren't doing what you expect → Press `ESC` to get back to command mode
- Then try again
:::
...
## Emergency: I'm Stuck in Vi! 😱 {.smaller}
...
## The Problem: Copy-Paste Programming {.small}
...
## Why Default Arguments? → Real Use Case {.smaller}
...
## Check Your Understanding 🤔 {.small}
...
## Why Files? → Saving Your Work {.small}
