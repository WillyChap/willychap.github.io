...
## The `__init__.py` File {.tiny}

When your code lives in a **folder** (not just loose `.py` files), Python needs a signal that the folder is a **package** you can import from.

::: {.fragment}
**That signal is `__init__.py`:**
...
::: {.fragment}
**What goes in it?** Usually nothing — an empty file is fine:
...
::: {.fragment}
**Rule:** Any folder you want to import from needs an `__init__.py`. It can be empty.
:::
...
|-------|----------------|-----------------|
| **(a)** Deep left lobe ($x < -10$, low $z$) | Deep in the left wing | Ring of uncertainty barely grows — **highly predictable** |
| **(b)** High left lobe ($x < -5$, high $z$) | Upper part of left wing, near transition | Ring distorts into banana/boomerang shapes — **unclear if it will switch lobes** |
| **(c)** Saddle region ($|x| < 2$) | Between the two wings | Ring **explodes** — some go left, some go right — **no predictability** |
:::
