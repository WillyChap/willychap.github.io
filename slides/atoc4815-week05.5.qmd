...
::: {.fragment}
**But then your midterm project looked like this:**
...
::: {.fragment}
**Rule of thumb:** Use `from module import name1, name2` for most cases. Use `import module` when you use many things from it (like `numpy`).
:::
...
::: {.fragment}
```
ImportError: cannot import name 'helper_a' from partially initialized
module 'file_a' (most likely due to a circular import)
```

**`file_a` imports `file_b`, which tries to import `file_a`... which isn't done loading yet!**
:::

::: {.fragment}
**Fix:** Restructure so the dependency goes one way, or move the shared code into a third file:

```
utils.py       ← shared helpers
file_a.py      ← imports from utils
file_b.py      ← imports from utils
```
:::
...
## Common Error: Importing From a Subfolder {.tiny}

**This layout won't work by default:**
...
```python
# main.py
from helpers.conversions import celsius_to_fahrenheit
# ModuleNotFoundError!
```

::: {.fragment}
**Fix:** Add an empty `__init__.py` to make `helpers/` a **package**:
...
```python
# main.py — now this works
from helpers.conversions import celsius_to_fahrenheit
```
:::

::: {.fragment}
**`__init__.py`** tells Python: "this folder is a package you can import from."

(In Python 3.3+ this is technically optional for "namespace packages", but **always include it** — it's the standard practice and avoids subtle bugs.)
:::
