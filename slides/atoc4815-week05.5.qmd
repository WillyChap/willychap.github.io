...
::: {.fragment}
**But then a .py for our Lorenz system looks like this:**
...
::: {.fragment}
**This is how python software code is organized** — the same pattern scales from homework to research.
:::
...
## The `__init__.py` File {.tiny .scrollable}
...
```bash
cd lorenz_project/
python run_lorenz_ensemble.py        # "from lorenz63 import Lorenz63" works fine
```
...
```bash
cd lorenz_lab/                       # one level UP from lorenz_project/
...
# ModuleNotFoundError: No module named 'lorenz_project'
```

**With `__init__.py` (even empty):**

```python
from lorenz_project.lorenz63 import Lorenz63  # works!
```
:::
...
## `__init__.py` — More Than Just Empty {.tiny .scrollable}
...
```python
from lorenz_project.lorenz63 import Lorenz63      # still works
```

users can just write:

```python
from lorenz_project import Lorenz63                # also works!
```
:::
...
```python
import lorenz_project
print(lorenz_project.__version__)   # "0.1.0"
```
:::
...
```python
# lorenz_project/__init__.py
__all__ = ["Lorenz63", "integrate"]   # only these get exported with *
```
:::
...
## Predictability Depends on Location {.tiny .scrollable}
...
![](lorenz_ensemble_predictability.png){width="100%"}
...
## File 2: `lorenz63.py` {.tiny .scrollable}
...
2. **Vectorized:** single loop over time steps, advance all members at once
...
## Deliverables Checklist {.tiny .scrollable}

::: {.incremental}
1. **`integrators.py`** — run `python -m lorenz_project.integrators` and confirm it prints `"all checks passed!"` (tests Euler on $dy/dt = -y$, checks answer ≈ $e^{-1}$)
2. **`lorenz63.py`** — run `python -m lorenz_project.lorenz63` and confirm it prints `"all checks passed!"` (checks trajectory shape is `(1001, 3)` and ensemble shape is `(5, 1001, 3)`)
3. **`plotting.py`** — run `python -m lorenz_project.plotting` — a plot window should appear with a random-walk line. If you see a line, it works.
4. **`run_lorenz_ensemble.py`** — run `python -m lorenz_project.run_lorenz_ensemble` — produces and saves `lorenz_ensemble_predictability.png`
5. **Every file** has an `if __name__ == "__main__":` guard with tests inside it
...
::: {.fragment}
You already wrote all the hard code for the midterm. Now you're organizing it and extending it.
:::
