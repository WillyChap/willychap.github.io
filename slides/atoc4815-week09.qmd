...
::: {.fragment}
**Bottom line:** For scientific computing, use **processes** (not threads) to get real parallelism.
:::
...
- NumPy loops in **compiled C code** under the hood
...
## Pool.map: The Basic Pattern {.tiny}
...
::: {.fragment .tiny}
**Note:** `multiprocessing` must be run from a `.py` script, not a notebook. Try this in your terminal!
:::
...
## Common Error: Forgetting the `__name__` Guard {.tiny}
...
```python
# Replace:   [f(x) for x in array]
# With:      np.vectorize(f)(array)  or better: direct NumPy ops
