...
print(f"15°C = {celsius_to_fahrenheit(15)}°F")
...
def format_temp(temp_c, unit="C", decimal_places=1):
    ...
    return f"{temp_value:.{decimal_places}f}°{unit}"
...
::: {.tiny}
Python technically returns one object → here that object is a tuple.
...
print(f"Mean only: {mean_only:.1f}°C")
```
:::
...
def describe_conditions(temp_c, wind_kmh):
    ...
    return f"Actual: {temp_c}°C, Feels like: {feels:.1f}°C ({category})"
...
- `compute_feels_like` → number
- `classify_temp` → label
- `describe_conditions` → combines them
...
print(station)
print(f"Mean temp: {station.mean_temp():.1f}°C")
```
...
print(f"{boulder.name}: {boulder.mean_temp():.1f}°C")
print(f"{denver.name}: {denver.mean_temp():.1f}°C")
```
...
class WeatherObservation:
    ...
    def __init__(self, temp_c, wind_kt, time):
        # Validate inputs
        if not (-50 <= temp_c <= 50):
            raise ValueError(f"Temperature {temp_c}°C is out of range")
        if wind_kt < 0:
    ...
    def __str__(self):
        return f"{self.time}: {self.temp_c}°C, {self.wind_kt}kt"

# Use it
obs = WeatherObservation(temp_c=-5, wind_kt=20, time="2026-01-15 12:00")
print(obs)
print(f"Feels like: {obs.feels_like():.1f}°C")
...
print(f"Mean temp: {dataset.mean_temp():.1f}°C")
...
print(f"Mean temperature: {data.mean_temp():.1f}°C")
