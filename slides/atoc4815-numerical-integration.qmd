...
1. **Analytical integration** → exact equation for $y(t)$
2. **Numerical integration** → approximation for $y(t)$
:::
...
2. **Know the method structure:**
   ...
   - `tendency()` → calculates $f(y)$
   - `step()` → applies Euler formula
   - `integrate()` → loops through time
...
ax1.plot(t_large, T_large, 'ro-', linewidth=2, markersize=8, label='Euler (Δt=1.0)')
ax1.set_xlabel('Time', fontsize=12)
ax1.set_ylabel('Temperature', fontsize=12)
ax1.set_title('Large Time Step (Δt=1.0)', fontsize=14)
...
ax2.plot(t_small, T_small, 'bo-', linewidth=1, markersize=4, label='Euler (Δt=0.1)')
ax2.set_xlabel('Time', fontsize=12)
ax2.set_ylabel('Temperature', fontsize=12)
ax2.set_title('Small Time Step (Δt=0.1)', fontsize=14)
...
dts = [0.5, 1.0, 1.5, 2.5]
titles = [
    r'$a\Delta t=0.5$ — smooth decay',
    r'$a\Delta t=1.0$ — one-step kill',
    r'$a\Delta t=1.5$ — oscillating decay',
    r'$a\Delta t=2.5$ — BLOW-UP',
]
...
for ax, dt, title, color in zip(axes.flat, dts, titles, colors):
    ...
    ax.plot(t_e, y_e, 'o-', color=color, lw=1.5, ms=5, label=f'Euler Δt={dt}')
...
ax1.plot(u_e, v_e, '-', color='#C44E52', lw=1.2, label=f'Euler (Δt={int(h)}s)')
...
plt.suptitle(r'Inertial oscillation: $f=10^{-4}$ s$^{-1}$, no drag, Δt=600s', fontsize=13)
plt.tight_layout()
plt.show()
```
:::
...
Define the squared speed (“radius squared”):
$$
R_n = u_n^2 + v_n^2
$$
...
ax1.plot(u_e, v_e, '--', color='#4A7C59', lw=1.2, label=f'Euler (Δt={int(h)}s)')
ax1.set_xlabel('u (m/s)', fontsize=11)
ax1.set_ylabel('v (m/s)', fontsize=11)
ax1.set_title('Phase space (u, v)', fontsize=12)
ax1.legend(fontsize=9)
...
plt.suptitle(r'Damped inertial oscillation: $f=10^{-4}$, $r=10^{-5}$ s$^{-1}$, Δt=600s', fontsize=13)
plt.tight_layout()
plt.show()
```
...
- ✓ Understand all four class methods (`__init__`, `tendency`, `step`, `integrate`)
- ✓ Know Lorenz63 structure (bonus: calculate by hand)
