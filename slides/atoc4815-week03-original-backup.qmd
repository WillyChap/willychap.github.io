...
**Get in the habit:** make plot → save with `savefig` → show
...
2. Counts how many hours exceed 20 °C and how many fall below 12 °C
...
print(f"Max: {max_temp:.1f}°C at hour {max_hour}")
print(f"Min: {min_temp:.1f}°C at hour {min_hour}")
print(f"Daily range: {daily_range:.1f}°C")

# 2. Counts
hot_hours = np.sum(observed > 20)
cold_hours = np.sum(observed < 12)
print(f"Hours > 20°C: {hot_hours}")
print(f"Hours < 12°C: {cold_hours}")
