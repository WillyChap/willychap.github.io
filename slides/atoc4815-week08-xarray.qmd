...
temp.mean(axis=1)
# ❌ Is axis=1 pressure or latitude?
...
# ❌ Easy to mix up order, loses metadata
```
...
# ❌ This might fail if exact value isn't in the grid
...
**Make a publication-quality map of the temperature data:**
...
# Tasks:
# 1. Plot the time-mean TREFHT using a Robinson projection
# 2. Add coastlines and country borders
# 3. Add a grid with labels
# 4. Try switching to ccrs.Orthographic(-105, 40)
#    (centered on Boulder!) — what changes?

# Bonus: plot the TREFHT ANOMALY (isel(time=5) minus time mean)
#        with a diverging colormap (cmap='RdBu_r') and vmin/vmax symmetric
```
...
fig, ax = plt.subplots(figsize=(10, 5),
                        subplot_kw={'projection': ccrs.Robinson()})
ds['TREFHT'].mean('time').plot(ax=ax, transform=ccrs.PlateCarree(),
                                cmap='plasma',
                                cbar_kwargs={'label': 'TREFHT (K)', 'shrink': 0.7})
...
1. Open a NetCDF file with `xr.open_dataset()`
...
5. Plot the resulting time series with axis labels and a title
6. Try a second variable and compare on the same plot
