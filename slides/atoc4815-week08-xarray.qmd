...
ax.set_title('2-meter Temperature', fontsize=13)
plt.tight_layout()
plt.show()
```
...
# Tasks:
# 1. Plot the time-mean temperature using a Robinson projection
...
# Bonus: plot the temperature ANOMALY (isel(time=5) minus time mean)
...
ds['t2m'].mean('time').plot(ax=ax, transform=ccrs.PlateCarree(),
                             cmap='plasma',
                             cbar_kwargs={'label': 'T (K)', 'shrink': 0.7})
