...
## Why Python for Atmospheric Science? {.small}
...
::: {.fragment}
⚠️ **Danger:** If condition never becomes False → infinite loop!
Make sure loop will eventually exit.
:::

# Functions {background-color="#9CA898"}

## Why Functions? → Reusable Code {.small}
...
::: {.fragment}
**Bugs:**
1. `range(len(stations) + 1)` → IndexError (goes to 4, but only 3 items)
...
## Next Week: NumPy Arrays {.scrollable}
...
🌐 willychap.github.io
🏢 SEEC Building (N258), East Campus
