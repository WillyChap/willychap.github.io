...
## The Water Budget Problem {.smaller}
...
## Mass Loss from Bilinear Interpolation {.smaller}
...
## The Key Idea: Area Overlap {.smaller}
...
## A Simple Manual Example {.smaller}
...
# Source: 2° cells centered at 1, 3, 5, 7, 9 degrees
# Target: 3° cells centered at 1.5, 4.5, 7.5 degrees
src_centers = np.array([1, 3, 5, 7, 9], dtype=float)
src_edges   = np.array([0, 2, 4, 6, 8, 10], dtype=float)
f_src       = np.array([4.0, 1.0, 8.0, 2.0, 6.0])  # e.g. precip mm/day

tgt_edges = np.array([0, 3, 6, 9], dtype=float)
...
## From 1D to 2D: Why Software Matters {.smaller}
...
## xESMF: The Production Tool {.smaller}
...
## Comparison: All Three Methods on Precipitation {.smaller}
...
w_f = np.cos(np.radians(ds_fine.lat))
w_c = np.cos(np.radians(ds_nn.lat))
...
## Lab: Conservation in Practice {.smaller}
...
**Task 3 (7 min) — Reflection:**

You are building a dataset comparing CMIP6 model precipitation (2.5° grid) to GPCP observations (2.5° grid, but different lat/lon centers). Your collaborator says "just use bilinear, the grids are almost the same resolution."

Write 3-4 sentences explaining whether you agree and what you would actually do.
